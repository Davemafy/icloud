...
BRIDGE = ROOT / "mt5" / "stable" / "InstitutionalSMC_DataBridge_v1_38_CompileSafeJournalTruth.mq5"
CORE = ROOT / "mt5" / "stable" / "InstitutionalSMC_DataBridge_v1_31_XAU_DXY_Journal.mq5"


def test_v138_reports_current_bridge_and_sequence_runtime_truth():
    bridge = BRIDGE.read_text(encoding="utf-8")
    core = CORE.read_text(encoding="utf-8")

    assert '#property version "1.38"' in bridge
    assert '#define TZ_BRIDGE_VERSION "1.38"' in bridge
    assert '#define TZ_SEQUENCE_EXPECTED "3.33"' in bridge
    assert '#define TZ_JOURNAL_BRIDGE_VERSION "1.38"' in bridge
    assert '\\n#define OnInit' not in bridge
    assert '\\n#ifndef TZ_JOURNAL_BRIDGE_VERSION' not in core
...
def test_v138_classifies_all_current_sequence_entry_tags():
    core = CORE.read_text(encoding="utf-8")
