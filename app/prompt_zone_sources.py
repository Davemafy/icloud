...
def liquidity_sweep_rejection_origins(
    ...
    for i in range(start, len(bars) - 1):
        ...
        sell_sweep = float(bar.high) > prior_high and float(bar.close) < prior_high
        ...
        if sell_sweep and sell_reject and sell_move:
        ...
        buy_sweep = float(bar.low) < prior_low and float(bar.close) > prior_low
        ...
        if buy_sweep and buy_reject and buy_move:
