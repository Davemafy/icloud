...
def build_prompt_candidates(snapshot: MarketSnapshot) -> list[Candidate]:
    """Build analysis candidates from both displacement and liquidity-sweep rejection origins.

    D1 remains context-only. H4 is the parent authority; H1 refines or acts as fallback.
    Downstream primary-zone policy still enforces structural BSL/SSL, freshness,
