...
def _payload(a: Analysis, s: MarketSnapshot) -> dict[str, Any]:
    return {
        ...
        "rules": {
            ...
            "m1_execution": "sweep -> MSS/BOS body close -> displacement -> new dealing range -> premium/discount -> OTE -> fresh PD array -> retracement",
            "risk": "AI never sets lot size",
        },
    }


SYSTEM = """You are a conservative validation layer for an XAUUSD institutional SMC engine.
The deterministic engine owns prices, zones, liquidity, invalidation and risk. You may approve/reject
or point out contradictions, but MUST NOT invent unseen market data, move zone prices, or prescribe lot size.
...
wait for retest + opposite MSS/BOS + displacement + premium/discount OTE + fresh PD array.
...
async def validate_with_ai(a: Analysis, s: MarketSnapshot) -> tuple[bool, str, list[str], str]:
    ...
    if SETTINGS.ai_compat_url and SETTINGS.ai_compat_key and SETTINGS.ai_compat_model:
        ...
        body = {
            ...
            "messages": [{"role": "system", "content": SYSTEM}, {"role": "user", "content": json.dumps(_payload(a, s))}],
            "temperature": 0.1,
