...
#property description "DEMO/PAPER XAU M1 execution: atomic handoff, post-sweep continuation, release-safe owner mirror."
...
#define TZ_SEQUENCE_VERSION "3.32"
...
void TZ28_ArmAcceptedFlip()
{
   ...
   Print("SMC Research v3.32: accepted invalidation captured BEFORE authority release. zone=",g_tzFlipSourceZone,
...
void TZ28_EvaluateAcceptedFlip()
{
   ...
   do
   {
      ...
      if(SendOrders(sig.buy,entry,sl,lots,true,tag,sig.pd_type))
      {
         ...
         TZ28_SaveAcceptedFlip();
         Print("SMC Research v3.32 ",tag," opened from accepted-zone flip. authority=",g_tzExecutionAuthority,
...
void Evaluate()
{
   ...
   if(SendOrders(sig.buy,entry,sl,lots,false,tag,sig.pd_type))
   {
      ...
      Print("SMC Research v3.32 ",tag," opened. authority=",g_tzExecutionAuthority," model=",g_tzLastModel,
...
int OnInit()
{
   ...
   Print("TradeZone Sequence runtime ",TZ_SEQUENCE_VERSION," active. Atomic handoff + post-sweep M1 confirmation + release-safe exact owner mirror; no chase.");return INIT_SUCCEEDED;
}
