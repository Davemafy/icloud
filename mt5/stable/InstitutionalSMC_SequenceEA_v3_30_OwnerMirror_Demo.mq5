...
#property description "DEMO/PAPER XAU M1 execution + MT5-local execution-owner persistence mirror for cloud restart recovery."
...
#define TZ_SEQUENCE_VERSION "3.30"
...
void TZ28_ArmAcceptedFlip()
{
   ...
   Print("SMC Research v3.30: accepted invalidation captured BEFORE authority release. zone=",g_tzFlipSourceZone,
...
void TZ28_EvaluateAcceptedFlip()
{
   ...
   do
   {
      ...
      if(SendOrders(sig.buy,entry,sl,lots,true,tag,sig.pd_type))
      {
         ...
         TZ28_SaveAcceptedFlip();
         Print("SMC Research v3.30 ",tag," opened from accepted-zone flip. authority=",g_tzExecutionAuthority,
...
void TZ30_SaveOwnerMirrorFromPlan(string text)
{
   ...
   if(KV(text,"owner_mirror_active")!="1")return;
   FolderCreate("TradeZone");
...
void TZ_SendSequenceHeartbeat()
{
   ...
   string body=StringFormat(
      ...
      "\"owner_mirror_reaction_confirmed_at\":%I64d,\"owner_mirror_best_price\":%s,"
...
void Evaluate()
{
   ...
   if(g_tzFlipPlanStored)
   {
      ...
      RefreshPlan();
      TZ27_LoadExecutionAuthority(false);
   ...
   {
      if(g_tzExecutionAuthority=="HTF_CORE_HANDOFF"||g_tzExecutionAuthority=="HTF_ZONE_SWEEP_HANDOFF")
      {
   ...
   if(!sig.valid)
   {
      string stage=((g_tzExecutionAuthority=="HTF_CORE_HANDOFF"||g_tzExecutionAuthority=="HTF_ZONE_SWEEP_HANDOFF")?
                    TZ_DiagnosePrimaryStage(r,a,origBuy,recentZone):"MICRO_PATTERN");
   ...
   if(SendOrders(sig.buy,entry,sl,lots,false,tag,sig.pd_type))
   {
      ...
      Print("SMC Research v3.30 ",tag," opened. authority=",g_tzExecutionAuthority," model=",g_tzLastModel,
...
int OnInit()
{
   ...
   TZ_SetGate("BOOT","READY");TZ28_LoadAcceptedFlip();TZ_PreCoreSync();TZ27_LoadExecutionAuthority(true);
   TZ28_ArmAcceptedFlip();TZ_SavePersistentState();TZ28_SaveAcceptedFlip();TZ_WriteSequenceState();TZ_SendSequenceHeartbeat();
   Print("TradeZone Sequence runtime ",TZ_SEQUENCE_VERSION," active. DEMO/PAPER execution owner is mirrored locally for cloud restart recovery; authority never bypasses M1 confirmation.");return INIT_SUCCEEDED;
}
...
{
   TZ28_ArmAcceptedFlip();
   TZ_PreCoreSync();TZ27_SeqCore_OnTimer();TZ_LoadPersistentState();TZ27_LoadExecutionAuthority(false);
