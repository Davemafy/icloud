...
#property description "DEMO/PAPER XAU M1 execution: confirmed-value reentries + auditable entry evidence + persistent runner risk."
...
#define TZ_SEQUENCE_VERSION "3.35"
...
void TZ35_SendEntryDecisionAudit(string tag,Signal &s,double entry,double sl,double lots,double a,int reactionIdx,MqlRates &r[])
{
   if(IsTester())return;
   datetime now=TimeTradeServer();if(now<=0)now=TimeCurrent();
   ...
   string setup=(StringFind(tag,"R")==0?"REENTRY_"+StringSubstr(tag,1):tag);
   ...
   string details=StringFormat(
      ...
      (ReentryRequireClosedM1ValueReaction?1:0),(reactionIdx>=1?1:0));
   string body=StringFormat(
...
void Evaluate()
{
   ...
   TZ35_SendEntryDecisionAudit(tag,sig,entry,sl,lots,a,valueReactionIdx,r);
