...
#property description "DEMO/PAPER XAU M1 execution: core or proven zone-sweep handoff + accepted-zone flip + frozen owner geometry."
...
#define TZ_SEQUENCE_VERSION "3.29"
...
void TZ28_ArmAcceptedFlip()
{
   ...
   Print("SMC Research v3.29: accepted invalidation captured BEFORE authority release. zone=",g_tzFlipSourceZone,
...
void TZ28_EvaluateAcceptedFlip()
{
   ...
   do
   {
      ...
      if(SendOrders(sig.buy,entry,sl,lots,true,tag,sig.pd_type))
      {
         ...
         TZ28_SaveAcceptedFlip();
         Print("SMC Research v3.29 ",tag," opened from accepted-zone flip. authority=",g_tzExecutionAuthority,
...
void Evaluate()
{
   ...
   if(SendOrders(sig.buy,entry,sl,lots,false,tag,sig.pd_type))
   {
      ...
      Print("SMC Research v3.29 ",tag," opened. authority=",g_tzExecutionAuthority," model=",g_tzLastModel,
...
int OnInit()
{
   ...
   Print("TradeZone Sequence runtime ",TZ_SEQUENCE_VERSION," active. DEMO/PAPER core-or-zone-sweep handoff + accepted-zone flip; authority never bypasses M1 confirmation.");return INIT_SUCCEEDED;
}
