...
#property description "DEMO/PAPER XAU M1 execution: safety-block owner persistence + atomic handoff + post-sweep continuation."
...
#define TZ_SEQUENCE_VERSION "3.33"
...
void TZ28_ArmAcceptedFlip()
{
   ...
   Print("SMC Research v3.33: accepted invalidation captured BEFORE authority release. zone=",g_tzFlipSourceZone,
...
void TZ28_EvaluateAcceptedFlip()
{
   ...
   do
   {
      ...
      if(SendOrders(sig.buy,entry,sl,lots,true,tag,sig.pd_type))
      {
         ...
         TZ28_SaveAcceptedFlip();
         Print("SMC Research v3.33 ",tag," opened from accepted-zone flip. authority=",g_tzExecutionAuthority,
...
void Evaluate()
{
   ...
   if(SendOrders(sig.buy,entry,sl,lots,false,tag,sig.pd_type))
   {
      ...
      Print("SMC Research v3.33 ",tag," opened. authority=",g_tzExecutionAuthority," model=",g_tzLastModel,
