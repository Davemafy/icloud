...
#property description "DEMO/PAPER XAU M1 execution: context x grade fixed-capital risk matrix + safe auto sizing + v3.36 confirmation/RR guards."
...
#define TZ_SEQUENCE_VERSION "3.38"
...
// v3.38 context x grade research risk. Quality grade and D1 alignment are
...
void ManagePositions()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      if(!trade.PositionModify(ticket,desired,tp))
         Print("SMC Research v3.38 management modify failed ticket=",ticket,
...
void TZ28_EvaluateAcceptedFlip()
{
   ...
   do
   {
      ...
      if(TZ37_SendOrders(sig.buy,entry,sl,lots,true,tag,sig.pd_type))
      {
         ...
         TZ28_SaveAcceptedFlip();
         Print("SMC Research v3.38 ",tag," opened from accepted-zone flip. authority=",g_tzExecutionAuthority,
...
void Evaluate()
{
   ...
   if(TZ37_SendOrders(sig.buy,entry,sl,lots,false,tag,sig.pd_type))
   {
      ...
      Print("SMC Research v3.38 ",tag," opened. authority=",g_tzExecutionAuthority," model=",g_tzLastModel,
