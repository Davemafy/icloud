...
#property description "DEMO/PAPER XAU M1 reaction-window execution + accepted-zone flip handoff independent of old-zone authority."

// Research wrapper around the validated v3.21 execution core.
// DEMO/PAPER ONLY. Real accounts remain hard-blocked.
// v3.28 keeps the v3.27 reaction-window model and fixes accepted-zone flips.
// M15 accepted invalidation is detected before cloud authority can disappear.
// The failed zone geometry is snapshotted locally, persisted, and may only earn
// ACCEPTED_ZONE_FLIP_HANDOFF after opposite-side retest + M1 structure/displacement/value.
// Invalidation itself is NEVER an entry. DEMO/PAPER ONLY.
...
#define TZ_SEQUENCE_VERSION "3.28"
...
void TZ28_ArmAcceptedFlip()
{
   ...
   Print("SMC Research v3.28: accepted invalidation captured BEFORE authority release. zone=",g_tzFlipSourceZone,
...
void TZ28_EvaluateAcceptedFlip()
{
   ...
   do
   {
      ...
      if(SendOrders(sig.buy,entry,sl,lots,true,tag,sig.pd_type))
      {
         ...
         TZ28_SaveAcceptedFlip();
         Print("SMC Research v3.28 ",tag," opened from accepted-zone flip. authority=",g_tzExecutionAuthority,
...
void Evaluate()
{
   ...
   if(SendOrders(sig.buy,entry,sl,lots,false,tag,sig.pd_type))
   {
      ...
      Print("SMC Research v3.28 ",tag," opened. authority=",g_tzExecutionAuthority," model=",g_tzLastModel,
...
int OnInit()
{
   ...
   Print("TradeZone Sequence runtime ",TZ_SEQUENCE_VERSION," active. DEMO/PAPER reaction-window + accepted-zone flip handoff; invalidation is never an instant entry.");return INIT_SUCCEEDED;
}
