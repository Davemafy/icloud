...
#property description "DEMO/PAPER XAU M1 execution: confirmed-value handoffs/reentries + minimum-RR/target-expiry guards + auditable entry evidence."
...
#define TZ_SEQUENCE_VERSION "3.36"
...
void ManagePositions()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      if(!trade.PositionModify(ticket,desired,tp))
         Print("SMC Research v3.36 management modify failed ticket=",ticket,
...
void TZ28_EvaluateAcceptedFlip()
{
   ...
   do
   {
      ...
      if(SendOrders(sig.buy,entry,sl,lots,true,tag,sig.pd_type))
      {
         ...
         TZ28_SaveAcceptedFlip();
         Print("SMC Research v3.36 ",tag," opened from accepted-zone flip. authority=",g_tzExecutionAuthority,
...
void Evaluate()
{
   ...
   if(SendOrders(sig.buy,entry,sl,lots,false,tag,sig.pd_type))
   {
      ...
      Print("SMC Research v3.36 ",tag," opened. authority=",g_tzExecutionAuthority," model=",g_tzLastModel,
