...
#property description "DEMO/PAPER bridge with Sequence 3.39 context-grade risk truth plus journal continuity recovery, V659 zones, and ownership."

#define TZ_JOURNAL_BRIDGE_VERSION "1.48"
...
#define TZ_BRIDGE_VERSION "1.48"
#define TZ_SEQUENCE_EXPECTED "3.39"
...
input int JournalContinuityResyncSeconds=180;
...
datetime g_tzLastJournalContinuitySync=0;
...
string TZ_SequenceExecutionDisplay()
{
   ...
   if(stage=="ENTRY_CONFIRMATION")return "WAITING FOR CLOSED M1 VALUE REACTION";
   ...
   if(stage=="FLIP_CONFIRMATION")return "WAITING FOR CLOSED M1 VALUE REACTION";
...
void TZ_RefreshPlanContextWithJournalContinuity()
{
   string previousCloudVersion=g_cloudVersion;
   RefreshPlanContext();

   datetime now=TimeTradeServer();
   bool cloudVersionChanged=(
      previousCloudVersion!="" &&
      g_cloudVersion!="" &&
      previousCloudVersion!=g_cloudVersion
   );
   int cadence=MathMax(60,JournalContinuityResyncSeconds);
   bool periodicDue=(
      g_tzLastJournalContinuitySync<=0 ||
      now-g_tzLastJournalContinuitySync>=cadence
   );

   // Cloud journal storage can restart independently of MT5. Replaying MT5
   // history is idempotent because cloud feedback uses stable deal/position
   // event_uid keys, so force an immediate recovery on a cloud-version change
   // and a light periodic continuity replay for same-version restarts.
   if(cloudVersionChanged || periodicDue)
   {
      BackfillJournalHistory(true);
      g_tzLastJournalContinuitySync=now;
   }
}
...
int OnInit()
{
   ...
   TZ_SendSnapshotCycle("V147_VERIFY");
   RefreshPlanContext();
   BackfillJournalHistory(true);
...
void OnTimer()
{
   ...
   TZ_RefreshPlanContextWithJournalContinuity();
