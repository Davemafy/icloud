...
#property description "DEMO/PAPER telemetry bridge with journal recovery, execution-truth chart status, legacy overlay cleanup, V659 zones, and ownership."

#define TZ_JOURNAL_BRIDGE_VERSION "1.40"
...
#define TZ_BRIDGE_VERSION "1.40"
#define TZ_SEQUENCE_EXPECTED "3.34"
...
int OnInit()
{
   ...
   TZ_SendSnapshotCycle("V140_VERIFY");
   RefreshPlanContext();
   TZ_ReadTwoZoneMap();
   TZR_RefreshAndRender();
   TZ_RenderTwoZoneMap();
   TZ_WriteBridgeState();
   TZ_SendHeartbeatV140();
   Print("AITS DataBridge runtime v",TZ_BRIDGE_VERSION," active: journal recovery + execution-truth overlay + legacy status cleanup + V659 zones + ownership visualization.");
